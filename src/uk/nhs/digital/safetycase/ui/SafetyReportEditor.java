...
"	}\n"
    ...
    private void exportButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_exportButtonActionPerformed
        ...
        data = data.replace("__HAZARDABLEPLACEHOLDER__", HAZARD_TABLE);
...
//                       
    ...
    private String systemFunctionReport(Persistable p) {
        ...
        if ((systemFunctions == null) && p.getDatabaseObjectName().contentEquals("SystemFunction")) {
            ...
            sfSB.append(P_START).append(String.format("This %s has no functions.", p.getDatabaseObjectName())).append(P_END);
            return sfSB.toString();
        }
            ...
            for (Relationship r : rels) {
                ...
                if ((m == null) || (m.contains("Diagram"))) { // controls creating two relations, filtering one.
                    ...
                    if (type.equals("Control")) { // control needs to show the state of the control
                        hdSB.append(P_START).append("<b>Title : </b>").append(p.getTitle()).append(" (").append(p.getAttributeValue("State")).append(")"). append("<br/>Description : </b>").append(description).append(P_END);
                    } else {
